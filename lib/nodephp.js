...
function makeHeaders(headers, params) {
	...
	debug.assert(headers).is('object');
	...
	return params.concat(Object.keys(headers).map(function(prop) {
 ...
 * using net.Socket and the fastcgi module.
 *
...
function server(request, response, params, options) {
    ...
    function sendRequest (connection) {
		...
		debug.assert(connection).is('object');
		debug.assert(connection.write).is('function');
		debug.assert(connection.end).is('function');
    ...
    connection.on("connect", function() {
		try {
			...
			if(writer) { throw new TypeError("writer loaded already!"); }
			if(parser) { throw new TypeError("parser loaded already!"); }
	        ...
	        parser.onRecord = function(record) {
				try {
					...
					debug.assert(record).is('object');
					debug.assert(record.header).is('object');

		            if (record.header.type == FCGI_STDOUT) {
						//debug.assert(record.body).is('object');
...
function serveStatic(file, path, response, request) {
   ...
   var types = {
    ...
    try {
        ...
        debug.log('  ---> Error loading file: ' + file);
        response.end();
    }
}
...
function serveFpm(script_file, script_dir, request, response, params, options) {
	...
	debug.log('script_file = ', script_file);
	debug.log('script_dir = ', script_dir);
	debug.log('params = ', params);
	debug.log('options = ', options);

	debug.assert(script_file).is('string');
	debug.assert(script_dir).is('string');
	debug.assert(request).is('object');
	debug.assert(response).is('object');
	debug.assert(params).is('array');
	debug.assert(options).is('object');
	...
	if(script_file[0] == '/') {
		script_file = script_file.substr(1);
	}

    try {
        fs.readFileSync(path.join(script_dir, script_file), 'utf8');
    } catch (err) {
        debug.log('  ---> Error loading file: ' + script_file);
    ...
    params = makeHeaders(request.headers, [
        ["SCRIPT_FILENAME", path.join(script_dir, script_file)],
        ["QUERY_STRING", qs],
...
/** Search the PHP file
 * @TODO This should be a lot smarter. There is PHP apps that use URLs like http://example.com/index.php/foobar
 */
function search_php_file(script_dir, u) {
	return u == '/' ? '/index.php' :  url.parse(u).pathname;
}

/**
 * Returns handler as `function(req, res)` 
 */
function createHandler(options) {
	debug.log('options = ', options);

	options = options || {};
    var script_dir = options.script_dir || (options.script_file && path.dirname(options.script_file)) || process.cwd();
	...
	return function(request, response) {
		...
		var script_file = (options.script_file && '/'+path.relative(script_dir, options.script_file)) || search_php_file(script_dir, request.url);
		serveFpm(script_file, script_dir, request, response, params, options);
    };
}
...
function createServer(opts, block) {
    ...
    http.createServer(function(request, response) {
        ...
        // script was "/". 
        var script_file = request.url[request.url.length] == '/' ? request.url + 'index.php' :  url.parse(request.url).pathname;
		...
		Object.keys(block).forEach(function(rule) {
    ...
    debug.log("PHP Server is now running on http://" + options.server.host + ":" + options.server.port);
}
